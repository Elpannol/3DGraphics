...
void ObjModel::draw()
{
    ...
    {
        glBindTexture(GL_TEXTURE_2D, materials[group->materialIndex]->texture->TextureId);
        for(Face face: group->faces)
        {
            glPushMatrix();
            for (Vertex vertex : face.vertices)
            {
                glTexCoord2f(texcoords[vertex.texcoord].x, texcoords[vertex.texcoord].y); glVertex3f(vertices[vertex.position].x, vertices[vertex.position].y, vertices[vertex.position].z);
            }
            glPopMatrix();
        }
    }
...
Texture::Texture(std::string file)
{
    ...
    {
        std::cout << "Succesfully loaded: " << file.c_str() << std::endl;
